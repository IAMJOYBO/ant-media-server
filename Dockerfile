...
RUN apt update && apt install -y apt-utils && apt install -y wget tree net-tools iputils-ping python3 python3-pip && apt-get clean &&  rm -rf /var/lib/apt/lists/*
...
RUN ./install_ant-media-server.sh ams_community.zip && rm -rf ams_community.zip && apt-get clean &&  rm -rf /var/lib/apt/lists/*
